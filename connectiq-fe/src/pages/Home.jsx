function Home() {
  return (
    <>
      <main>
        <section id="carousel">
          <div className="container-special">
            <div className="carousel-content">
              <div className="category-list">
                <ul>
                  <li>Beras dan biji-bijian</li>
                  <li>Minyak dan bumbu dapur</li>
                  <li>Protein Hewani</li>
                  <li>Sayur-sayuran</li>
                  <li>Buah-buahan</li>
                  <li>Minuman dan susu</li>
                </ul>
              </div>
              <div className="divider"></div>
              <div className="carousel-list">
                <img src="/assets/carousel.png" alt="" />
              </div>
            </div>
          </div>
        </section>
        <div className="container">
          <div className="section-divider"></div>
        </div>
        <section id="categories">
          <div className="container">
            <div className="categories-upper">
              <div className="red-title">
                <div className="red-block"></div>
                <p>Categories</p>
              </div>
              <h2>Browse By Category</h2>
            </div>
            <div className="categories-content">
              <div className="categories-card">
                <img src="/assets/Broccoli.png" alt="" />
                <p>Sayur-sayuran</p>
              </div>
              <div className="categories-card">
                <img src="/assets/Rice Vinegar.png" alt="" />
                <p>Minuman dan Susu</p>
              </div>
              <div className="categories-card">
                <img src="/assets/Spoon of Sugar.png" alt="" />
                <p>Minyak dan bumbu</p>
              </div>
              <div className="categories-card">
                <img src="/assets/Watermelon.png" alt="" />
                <p>Buah-buahan</p>
              </div>
              <div
                className="categories-card"
                style={{
                  backgroundColor: "#DB4444",
                  border: "1px solid #DB4444",
                }}
              >
                <img src="/assets/Thanksgiving.png" alt="" />
                <p>Protein hewani</p>
              </div>
              <div className="categories-card">
                <img src="/assets/Category-CellPhone.png" alt="" />
                <p>Beras dan bijian</p>
              </div>
            </div>
          </div>
        </section>
        <div className="container">
          <div className="section-divider"></div>
        </div>
        <section id="benefits">
          <div className="container">
            <div className="benefits-row">
              <div className="benefits-content">
                <div className="benefits-card">
                  <img src="/assets/Services.png" alt="" />
                  <h3>FREE AND FAST DELIVERY</h3>
                  <p>Free delivery for all orders over 1.000.000 IDR</p>
                </div>
                <div className="benefits-card">
                  <img src="/assets/Services (1).png" alt="" />
                  <h3>24/7 CUSTOMER SUPPORT</h3>
                  <p>Friendly 24/7 customer support</p>
                </div>
                <div className="benefits-card">
                  <img src="/assets/Services (2).png" alt="" />
                  <h3>MONEY BACK GUARANTEE</h3>
                  <p>We return money within 30 days</p>
                </div>
              </div>
            </div>
          </div>
        </section>
      </main>
    </>
  );
}

export default Home;
